...
class Discover_model extends CI_Model {
	...
	public function createViews($data) {
		...
		$firstQuery = "CREATE VIEW tmpView AS (SELECT pid, pname, description, pic, category AS cid FROM Project WHERE active=1 AND category=".$cid.");";
		...
		$query = $this->db->query("CREATE VIEW mostPopularInCid AS (SELECT Pro.pid AS pid, Pro.pname AS pname, Pro.description AS description, Pro.pic AS pic, Pro.cname AS cname, AVG(RP.rating) AS netAvg FROM projectsInCid Pro LEFT JOIN RateProj RP ON Pro.pid=RP.pid GROUP BY Pro.pid LIMIT 3);");
	}
